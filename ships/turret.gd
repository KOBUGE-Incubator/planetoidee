...
func should_trust(engine):
	return false # It's a turret. Really.
...
func should_fire(weapon):
	...
	for player in get_tree().get_nodes_in_group(target_group):
		var time_estimate = player.get_global_pos().distance_to(vantage) / shot_speed
		var target = player.get_global_pos() + player.get_linear_velocity() * time_estimate
		var distance = vantage.distance_to(target)
		if (target - vantage).normalized().dot(direction) > 1 - 0.7 / distance:
...
func _integrate_forces(state):
	...
	for player in get_tree().get_nodes_in_group(target_group):
		...
		if min_target == null or distance_squared < min_distance_squared:
